document.addEventListener("DOMContentLoaded", () => {
    ...
    function displayQuestion() {
        ...
        questionElement.innerHTML = `🎯 <strong>Question ${currentQuestionIndex + 1} of 15:</strong> ${decodeHTMLEntities(questionData.question)}`;
        ...
        answers.forEach(answer => {
            ...
            input.type = "radio";
            input.name = "quiz-option";  // Fix: Ensure the correct name attribute
            input.value = decodeHTMLEntities(answer);
    ...
    submitButton.addEventListener("click", () => {
        ...
        if (!selectedOption) {
            // Show a warning message but keep the timer running
            const warning = document.getElementById("warning-message");
            if (!warning) {
                const msg = document.createElement("p");
                msg.id = "warning-message";
                msg.style.color = "red";
                msg.textContent = "⚠️ Please select an answer first!";
        ...
        // Remove the warning message if it was displayed
        const warning = document.getElementById("warning-message");
        if (warning) {
            warning.remove();
        }
