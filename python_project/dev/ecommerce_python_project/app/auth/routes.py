...
def signin_for_access_token(
    ...
    logger.info(
        f"User '{user.email}' signed in successfully."
...
def refresh_access_token(
    refresh_token: str = Depends(
        utils.oauth2_scheme
    ),  # We can reuse the scheme to get the token from the header # type: ignore
...
def perform_password_reset(
    request_data: schemas.ResetPasswordSchema, db: Session = Depends(get_db)
):
    """
